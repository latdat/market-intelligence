...
Persistence semantics (DE-012):

- stable shared candidate ID remains generated by DE-011;
- durable logical uniqueness is `(user_id, article_id)`;
- persistence is first-write-wins;
- a replay can recompute a different ephemeral `matched_at`/score/reasons, but DE-012 returns the
  original persisted candidate and does not mutate it;
